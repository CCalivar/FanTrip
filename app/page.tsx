...
const matches = [
  { id: 1, home: "Real Madrid", away: "Atletico Madrid", league: "LaLiga", date: "12 May", venue: "Bernabeu", city: "Madrid", price: 89, oldPrice: 108, drop: -18, emoji: "🔥", featured: true, tripFrom: 284 },
  { id: 2, home: "UCL Final", away: "TBD vs TBD", league: "Champions League", date: "31 May", venue: "Allianz Arena", city: "Munich", price: 210, oldPrice: 210, drop: null, emoji: "🏆", featured: true, tripFrom: 384 },
  { id: 3, home: "Man City", away: "Man United", league: "Premier League", date: "18 May", venue: "Etihad", city: "Manchester", price: 145, oldPrice: 163, drop: -11, emoji: "⚡", featured: false, tripFrom: 310 },
  { id: 4, home: "Arsenal", away: "Chelsea", league: "Premier League", date: "24 May", venue: "Emirates", city: "London", price: 67, oldPrice: 67, drop: null, emoji: "📈", featured: false, tripFrom: 190 },
  { id: 5, home: "Barcelona", away: "Real Madrid", league: "LaLiga", date: "3 Jun", venue: "Camp Nou", city: "Barcelona", price: 175, oldPrice: 220, drop: -20, emoji: "🌟", featured: false, tripFrom: 340 },
];
    ...
    <main style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", background: "#080808", minHeight: "100vh", color: "#fff", overflowX: "hidden" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 32px", background: "rgba(8,8,8,0.85)", backdropFilter: "blur(20px)", borderBottom: "0.5px solid rgba(255,255,255,0.06)" }}>
        ...
        <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
          ...
          <button style={{ background: "linear-gradient(135deg, #dc2626, #f97316)", color: "#fff", border: "none", borderRadius: 8, padding: "7px 16px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Pro — 5€/mo</button>
        </div>
      </nav>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 0, maxWidth: 560, margin: "0 auto 56px", background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {featured.map(m => (
              <div key={m.id} onClick={() => router.push(`/match/${m.id}`)}
                ...
                <div style={{ position: "absolute", top: -20, right: -20, width: 120, height: 120, borderRadius: "50%", background: m.id === 2 ? "rgba(251,191,36,0.06)" : "rgba(220,38,38,0.06)", pointerEvents: "none" }} />
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: 0.4, color: m.id === 2 ? "#fbbf24" : "#f87171", textTransform: "uppercase" }}>{m.league}</div>
                  <span style={{ fontSize: 22 }}>{m.emoji}</span>
                </div>
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.2, marginBottom: 4 }}>{m.home}</div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", borderTop: "0.5px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 600, color: m.id === 2 ? "#fbbf24" : "#f87171" }}>{m.tripFrom}€</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div style={{ padding: "24px 24px 0", maxWidth: 760, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.1em" }}>Price drops this week</span>
            ...
            {rest.map(m => (
              <div key={m.id} onClick={() => router.push(`/match/${m.id}`)}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)" }}>{m.date} · {m.venue} · <span style={{ color: "rgba(255,255,255,0.45)" }}>{m.league}</span></div>
                </div>
