...
export default function MatchPage() {
  ...
  useEffect(() => {
    // Buscar datos del partido desde la API de matches
    fetch("/api/matches")
      .then(r => r.json())
      .then(data => {
        const found = Array.isArray(data) ? data.find((m: any) => String(m.id) === String(id)) : null;
        setMatch(found || { home: "Home Team", away: "Away Team", league: "League", date: "TBC", venue: "TBC", city: "Europe", price: 89, tripFrom: 284 });
      });
  }, [id]);
...
const flights = [
  ...
  if (!match) return <div style={{ background: "#F2F3F5", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter,sans-serif", color: "#aaa" }}>Loading...</div>;
    ...
    <main style={{ fontFamily: "'Inter','Helvetica Neue',sans-serif", background: "#F2F3F5", minHeight: "100vh", color: "#1a1a1a" }}>
      ...
      <nav style={{ background: "#0D0D0D", height: 56, display: "flex", alignItems: "center", padding: "0 28px", borderBottom: "1.5px solid #E8330A", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "rgba(255,255,255,0.4)", marginLeft: "auto", marginRight: 24 }}>
          <span onClick={() => router.push("/")} style={{ cursor: "pointer", color: "rgba(255,255,255,0.4)" }}>Home</span>
          <span style={{ color: "rgba(255,255,255,0.2)" }}>›</span>
          <span style={{ color: "rgba(255,255,255,0.4)" }}>{match.league}</span>
          <span style={{ color: "rgba(255,255,255,0.2)" }}>›</span>
      ...
      <div style={{ background: "linear-gradient(160deg,#0D0D0D 0%,#1a1a2e 60%,#16213e 100%)", padding: "28px 28px 0", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 14, color: "rgba(255,255,255,0.45)", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
            <span>📅 {match.date}</span>
            <span style={{ color: "rgba(255,255,255,0.2)" }}>·</span>
            <span>🏟 {match.venue}</span>
            <span style={{ color: "rgba(255,255,255,0.2)" }}>·</span>
            <span>📍 Prices from Madrid (MAD)</span>
          </div>
      ...
      {/* CONTENT */}
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "20px 28px 60px", display: "grid", gridTemplateColumns: "1fr 320px", gap: 20 }}>

        {/* LEFT */}
        <div>

          {/* TOTAL CARD */}
          <div style={{ background: "#fff", borderRadius: 14, border: "2px solid #E8330A", padding: "20px 24px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 4px 20px rgba(232,51,10,0.1)" }}>
            <div>
              <div style={{ fontSize: 11, color: "#999", marginBottom: 5, display: "flex", alignItems: "center", gap: 6 }}>
                Best combination · 1 person · 2 nights
                <span style={{ background: "#FFF5F3", color: "#E8330A", fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 999 }}>📍 from Madrid</span>
              </div>
              <div style={{ fontSize: 40, fontWeight: 800, color: "#1a1a1a", letterSpacing: -2, lineHeight: 1, marginBottom: 4 }}>{totalPrice}€</div>
              <div style={{ fontSize: 12, color: "#aaa", display: "flex", gap: 6, alignItems: "center" }}>
                <span>Ticket {ticketList[0]?.price}€</span><span style={{ color: "#ddd" }}>+</span>
                <span>Flight {flights[2].price}€</span><span style={{ color: "#ddd" }}>+</span>
                <span>Hotel {hotels[0].price * 2}€</span>
              </div>
              ...
              {ticketList.map((t, i) => (
                <div key={i} style={{ padding: "14px 18px", borderBottom: i < ticketList.length - 1 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14, background: i === 0 ? "#FAFFFE" : "#fff" }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <a href={t.url} target="_blank" rel="noopener noreferrer" style={{ marginTop: 6, padding: "5px 12px", fontSize: 11, fontWeight: 600, borderRadius: 7, cursor: "pointer", background: i === 0 ? "#E8330A" : "transparent", border: i === 0 ? "none" : "1.5px solid #e0e0e0", color: i === 0 ? "#fff" : "#666", textDecoration: "none", display: "inline-block" }}>Buy on {t.platform} →</a>
                  </div>
                </div>
              ))}
            </div>
            ...
            <>
              <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", marginBottom: 12, border: "1px solid #ebebeb" }}>
                <div style={{ padding: "14px 18px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: "#aaa", marginLeft: "auto" }}>MAD → {match.city} · estimated</span>
                </div>
                {flights.map((f, i) => (
                  <div key={i} style={{ padding: "14px 18px", borderBottom: i < flights.length - 1 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14, background: i === 0 ? "#FAFFFE" : "#fff" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3, display: "flex", alignItems: "center", gap: 8 }}>
                        {f.airline} · {f.route}
                        {f.badge && <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 999, background: i === 0 ? "#E3F2FD" : "#E8F5E9", color: i === 0 ? "#0D47A1" : "#1B5E20", fontWeight: 700 }}>{f.badge}</span>}
                      </div>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <a href={f.url} target="_blank" rel="noopener noreferrer" style={{ marginTop: 6, padding: "5px 12px", fontSize: 11, fontWeight: 600, borderRadius: 7, cursor: "pointer", background: "transparent", border: "1.5px solid #e0e0e0", color: "#666", textDecoration: "none", display: "inline-block" }}>Book on Skyscanner →</a>
                    </div>
                  </div>
                ))}
                {/* NEARBY AIRPORTS */}
                <div style={{ background: "#EFF8FF", border: "1px solid #BFDFFF", borderRadius: 10, padding: "13px 16px", margin: "12px 18px", display: "flex", alignItems: "flex-start", gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#1565C0", marginBottom: 10, lineHeight: 1.4 }}>Flying from a different airport near Madrid can reduce your flight cost.</div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {[["✈ Valladolid (VLL)", "Save 43€", "190km"], ["✈ Zaragoza (ZAZ)", "Save 31€", "320km"], ["✈ Barcelona (BCN)", "Save 18€", "Train 2h30"]].map(([airport, saving, dist]) => (
                        <div key={airport} style={{ background: "#fff", border: "1.5px solid #90CAF9", borderRadius: 8, padding: "7px 12px", cursor: "pointer" }}>
                          <div style={{ fontSize: 12, fontWeight: 700, color: "#1a1a1a" }}>{airport}</div>
              ...
              <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", marginBottom: 12, border: "1px solid #ebebeb" }}>
                <div style={{ padding: "14px 18px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: "#aaa", marginLeft: "auto" }}>2 nights · via Booking.com</span>
                </div>
                {hotels.map((h, i) => (
                  <div key={i} style={{ padding: "14px 18px", borderBottom: i < hotels.length - 1 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14, background: i === 0 ? "#FFFDF5" : "#fff" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3, display: "flex", alignItems: "center", gap: 8 }}>
                        {h.name}
                        {h.badge && <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 999, background: i === 0 ? "#FFF8E1" : "#FFEBEE", color: i === 0 ? "#F57F17" : "#B71C1C", fontWeight: 700 }}>{h.badge}</span>}
                      </div>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 800 }}>{h.price}€<span style={{ fontSize: 11, color: "#aaa", fontWeight: 400 }}>/nt</span></div>
                      <a href={h.url} target="_blank" rel="noopener noreferrer" style={{ marginTop: 6, padding: "5px 12px", fontSize: 11, fontWeight: 600, borderRadius: 7, cursor: "pointer", background: "transparent", border: "1.5px solid #e0e0e0", color: "#666", textDecoration: "none", display: "inline-block" }}>Book on Booking →</a>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", marginBottom: 12, border: "1px solid #ebebeb" }}>
                <div style={{ padding: "14px 18px", borderBottom: "1px solid #f0f0f0", background: "linear-gradient(135deg,#FAF5FF,#F5F0FF)", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: "#9C27B0", marginLeft: "auto", fontWeight: 500 }}>Recommended for away fans</span>
                </div>
                {[
                  ...
                  { icon: "🚗", bg: "#E0F2F1", name: "Airport Transfer", tag: "Convenient", tagBg: "#E0F2F1", tagColor: "#00695C", desc: "Book in advance and avoid match-day surcharges.", price: "from 28€", sub: "per vehicle", btnText: "Book transfer →", btnBg: "#00695C", url: "https://www.gettransfer.com" },
                ].map((extra, i) => (
                  <div key={i} style={{ padding: "14px 18px", borderBottom: i < 3 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#aaa", lineHeight: 1.4 }}>{extra.desc}</div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 800, color: "#1a1a1a" }}>{extra.price}</div>
                      <div style={{ fontSize: 10, color: "#aaa", marginBottom: 6 }}>{extra.sub}</div>
                      <a href={extra.url} target="_blank" rel="noopener noreferrer" style={{ padding: "6px 14px", fontSize: 11, fontWeight: 700, borderRadius: 7, cursor: "pointer", background: extra.btnBg, color: "#fff", textDecoration: "none", display: "inline-block", border: "none" }}>{extra.btnText}</a>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === "history" && (
            <div style={{ background: "#fff", borderRadius: 14, padding: "20px 24px", border: "1px solid #ebebeb" }}>
              ...
              <div style={{ fontSize: 11, color: "#aaa", marginBottom: 16 }}>Last 4 weeks · average price</div>
              <svg width="100%" height="80" viewBox="0 0 600 80" preserveAspectRatio="none">
                ...
                <circle cx="375" cy="62" r="4" fill="#2E7D32"/>
              </svg>
          ...
          {activeTab === "alerts" && (
            <div style={{ background: "#fff", borderRadius: 14, padding: "24px", border: "1px solid #ebebeb" }}>
              ...
              <div style={{ fontSize: 13, color: "#aaa", marginBottom: 24, lineHeight: 1.6 }}>Track tickets, flights and hotels for this match. We'll email you when any drops below your target.</div>
              {alertSet ? (
                <div style={{ background: "#E8F5E9", border: "1px solid #A5D6A7", borderRadius: 10, padding: "24px", textAlign: "center" }}>
                  <div style={{ fontSize: 32, marginBottom: 12 }}>✅</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: "#2E7D32", marginBottom: 6 }}>Alert set for {alertPrice}€</div>
                  <div style={{ fontSize: 13, color: "#aaa" }}>We'll notify {alertEmail} when prices drop.</div>
                </div>
              ) : (
                <>
        ...
        <div>
          <div style={{ background: "#fff", borderRadius: 14, padding: "18px", marginBottom: 14, border: "2px solid #E8330A" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, color: "#1a1a1a", marginBottom: 4, letterSpacing: -0.3 }}>Alert me when it drops</div>
            <div style={{ fontSize: 12, color: "#aaa", marginBottom: 14, lineHeight: 1.5 }}>Get notified when the full trip price drops below your target.</div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "16px 18px", marginBottom: 14, border: "1px solid #ebebeb" }}>
            ...
            {[
              ...
              { icon: "🚗", bg: "#E0F2F1", name: "Airport Transfer", desc: "CDG to stadium · from 28€", btn: "Book →", btnBg: "#00695C", url: "https://www.gettransfer.com" },
            ].map((item, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: i < 3 ? "1px solid #f7f7f7" : "none" }}>
                ...
                <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, fontWeight: 700, padding: "5px 10px", borderRadius: 6, cursor: "pointer", background: item.btnBg, color: "#fff", textDecoration: "none", border: "none", whiteSpace: "nowrap" }}>{item.btn}</a>
              </div>
            ))}
          </div>

          <div style={{ background: "#fff", borderRadius: 14, padding: "16px 20px", border: "1px solid #ebebeb" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#1a1a1a", marginBottom: 12 }}>Share this match</div>
