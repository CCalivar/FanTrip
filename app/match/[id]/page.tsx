...
function buildAviasalesUrl(originIata: string, destIata: string, date: string) {
  try {
    const d = new Date(date + " 2026");
    const day = String(d.getDate()).padStart(2, "0");
    const month = String(d.getMonth() + 1).padStart(2, "0");
    const search = `https://aviasales.com/search/${originIata}${day}${month}${destIata}1`;
...
export default function MatchPage() {
  ...
  const aviasalesUrl = buildAviasalesUrl(originIata, destIata, match?.date || "");
  const bookingUrl = (() => {
    try {
      const d = new Date(match?.date + " 2026");
      const checkin = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
      const d2 = new Date(d.getTime() + 2 * 86400000);
      const checkout = `${d2.getFullYear()}-${String(d2.getMonth()+1).padStart(2,"0")}-${String(d2.getDate()).padStart(2,"0")}`;
      return `https://www.booking.com/search.html?ss=${encodeURIComponent(match?.city || "")}&checkin=${checkin}&checkout=${checkout}&group_adults=1&no_rooms=1`;
    } catch {
      return `https://www.booking.com/search.html?ss=${encodeURIComponent(match?.city || "")}`;
    }
  })();
  ...
  const ticketList = tickets.length > 0 ? tickets : [
    { platform: "StubHub", section: "Category B — Best available", price: match?.price || 89, oldPrice: null, badge: "Best value", delivery: "E-ticket · Instant", url: `https://stubhubinternational.sjv.io/gRWRRv?u=${encodeURIComponent(`https://www.stubhubinternational.com/search?q=${match?.home || ""}`)}` },
    { platform: "Viagogo", section: "Category A — Premium seats", price: (match?.price || 89) + 80, oldPrice: null, badge: null, delivery: "E-ticket · Instant", url: `https://www.viagogo.com/search?q=${encodeURIComponent(match?.home || "")}` },
    { platform: "Ticketmaster", section: "Official tickets", price: (match?.price || 89) + 40, oldPrice: null, badge: "Official", delivery: "E-ticket", url: `https://www.ticketmaster.com/search?q=${encodeURIComponent(match?.home || "")}` },
  ];

  const flightList = flights.length > 0 ? flights : [
    { airline: "Vueling", route: `${departureCity} → ${match?.city || "TBC"}`, duration: "Direct · 2h", price: 72, oldPrice: 94, badge: "Cheapest", url: aviasalesUrl },
    { airline: "Iberia", route: `${departureCity} → ${match?.city || "TBC"}`, duration: "Direct · 2h", price: 96, oldPrice: 117, badge: "↓ 18%", url: aviasalesUrl },
    { airline: "Ryanair", route: `${departureCity} → ${match?.city || "TBC"}`, duration: "Direct · 2h 30m", price: 54, oldPrice: null, badge: "Early flight", url: aviasalesUrl },
  ];
  ...
  const handleAlertSubmit = async () => {
    ...
    try {
      await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({ email: alertEmail, tags: [`match-${id}`, "price-alert"] }),
      });
    } catch {}
    setAlertSet(true);
  };
    ...
    <main style={{ fontFamily: "'Inter','Helvetica Neue',sans-serif", background: "#F2F3F5", minHeight: "100vh", color: "#1a1a1a" }}>
      ...
      <div style={{ background: "linear-gradient(160deg,#0D0D0D 0%,#1a1a2e 60%,#16213e 100%)", padding: "28px 20px 0", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", marginBottom: 20, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" as const }}>
            <span>📅 {match.date}</span>
            <span style={{ color: "rgba(255,255,255,0.2)" }}>·</span>
            <span>🏟 {match.venue}</span>
            <span style={{ color: "rgba(255,255,255,0.2)" }}>·</span>
            <span onClick={() => { const city = prompt("Your departure city (e.g. Madrid, London, Paris):"); if (city) { localStorage.setItem("departureCity", city); setDepartureCity(city); } }}
        ...
        <div className="match-grid" style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20 }}>
          <div>
            ...
            {(activeTab === "trip" || activeTab === "tickets") && (
              <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", marginBottom: 12, border: "1px solid #ebebeb" }}>
                ...
                {ticketList.map((t, i) => (
                  <div key={i} style={{ padding: "14px 18px", borderBottom: i < ticketList.length - 1 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14, background: i === 0 ? "#FAFFFE" : "#fff", flexWrap: "wrap" as const }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" as const }}>
                        {t.section}
              ...
              <>
                {isSameCity ? (
                  ...
                  <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", marginBottom: 12, border: "1px solid #ebebeb" }}>
                    ...
                    {flightList.map((f, i) => (
                      <div key={i} style={{ padding: "14px 18px", borderBottom: i < flightList.length - 1 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14, background: i === 0 ? "#FAFFFE" : "#fff", flexWrap: "wrap" as const }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" as const }}>
                            {f.airline} · {f.route}
                        ...
                        <div style={{ textAlign: "right" as const }}>
                          ...
                          <a href={f.url} target="_blank" rel="noopener noreferrer" style={{ marginTop: 6, padding: "5px 12px", fontSize: 11, fontWeight: 600, borderRadius: 7, background: "transparent", border: "1.5px solid #e0e0e0", color: "#666", textDecoration: "none", display: "inline-block" }}>Search on Aviasales →</a>
                        </div>
                      </div>
                    ))}
                ...
                <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", marginBottom: 12, border: "1px solid #ebebeb" }}>
                  ...
                  {hotelList.map((h, i) => (
                    <div key={i} style={{ padding: "14px 18px", borderBottom: i < hotelList.length - 1 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14, background: i === 0 ? "#FFFDF5" : "#fff", flexWrap: "wrap" as const }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" as const }}>
                          {h.name}
                ...
                <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", marginBottom: 12, border: "1px solid #ebebeb" }}>
                  ...
                  {[
                    ...
                    { icon: "📱", bg: "#E3F2FD", name: "eSIM for Europe", tag: "🔥 Popular", tagBg: "#E3F2FD", tagColor: "#0D47A1", desc: "Stay connected without roaming. 5GB from 4.50€.", price: "from 4.50€", sub: "5GB · 30 days", btnText: "Get Airalo →", btnBg: "#1565C0", url: "https://tp.media/r?marker=723474.FT&trs=523774&p=8310&u=https%3A%2F%2Fairalo.com&campaign_id=541" },
                    { icon: "📱", bg: "#E0F2F1", name: "YeSIM — alternative", tag: "🌍 150+ countries", tagBg: "#E0F2F1", tagColor: "#00695C", desc: "Stay connected without roaming. From 3.50€.", price: "from 3.50€", sub: "150+ countries", btnText: "Get YeSIM →", btnBg: "#00695C", url: "https://tp.media/r?marker=723474.FT&trs=523774&p=5998&u=https%3A%2F%2Fyesim.tech&campaign_id=224" },
                    ...
                    { icon: "🚗", bg: "#E0F2F1", name: "Airport Transfer", tag: "Convenient", tagBg: "#E0F2F1", tagColor: "#00695C", desc: "Book in advance and avoid match-day surcharges.", price: "from 28€", sub: "per vehicle", btnText: "Book transfer →", btnBg: "#00695C", url: "https://tp.media/r?marker=723474.FT&trs=523774&p=4439&u=https%3A%2F%2Fgettransfer.com&campaign_id=147" },
                  ].map((extra, i) => (
                    <div key={i} style={{ padding: "14px 18px", borderBottom: i < 4 ? "1px solid #f7f7f7" : "none", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" as const }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" as const }}>
                          {extra.name}
            ...
            {activeTab === "alerts" && (
              <div style={{ background: "#fff", borderRadius: 14, padding: "24px", border: "1px solid #ebebeb" }}>
                <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Get the best deals first</div>
                <div style={{ fontSize: 13, color: "#aaa", marginBottom: 24, lineHeight: 1.6 }}>Join fans already tracking prices. We'll send you the best trip deals before they sell out.</div>
                {alertSet ? (
                  <div style={{ background: "#E8F5E9", border: "1px solid #A5D6A7", borderRadius: 10, padding: "24px", textAlign: "center" as const }}>
                    <div style={{ fontSize: 32, marginBottom: 12 }}>✅</div>
                    <div style={{ fontSize: 16, fontWeight: 700, color: "#2E7D32" }}>You're in!</div>
                    <div style={{ fontSize: 13, color: "#aaa", marginTop: 4 }}>We'll send the best deals to {alertEmail}.</div>
                  </div>
                ) : (
                  <>
          ...
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 14, padding: "16px 18px", marginBottom: 14, border: "1px solid #ebebeb" }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#1a1a1a", textTransform: "uppercase" as const, letterSpacing: "0.05em", marginBottom: 14 }}>Complete your trip</div>
              {[
                ...
                { icon: "📱", bg: "#E3F2FD", name: "Airalo eSIM", desc: "5GB from 4.50€ · no roaming", btn: "Get →", btnBg: "#1565C0", url: "https://tp.media/r?marker=723474.FT&trs=523774&p=8310&u=https%3A%2F%2Fairalo.com&campaign_id=541" },
                { icon: "📱", bg: "#E0F2F1", name: "YeSIM", desc: "From 3.50€ · 150+ countries", btn: "Get →", btnBg: "#00695C", url: "https://tp.media/r?marker=723474.FT&trs=523774&p=5998&u=https%3A%2F%2Fyesim.tech&campaign_id=224" },
                ...
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: i < 4 ? "1px solid #f7f7f7" : "none" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 14, padding: "16px 20px", border: "1px solid #ebebeb" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {["🔗 Copy", "𝕏 Post", "💬 Share"].map(btn => (
                  <button key={btn} onClick={() => btn === "🔗 Copy" && navigator.clipboard?.writeText(window.location.href)} style={{ flex: 1, padding: 9, border: "1.5px solid #e8e8e8", borderRadius: 8, fontSize: 11, fontWeight: 600, color: "#666", cursor: "pointer", background: "#fff", fontFamily: "inherit" }}>{btn}</button>
                ))}
              </div>
            </div>
          </div>
